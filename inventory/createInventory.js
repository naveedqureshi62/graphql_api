...
const resolvers = {
  ...
  Mutation: {
    addInventory: async (_, { inventory_id, inventory_details}) => {
      const pgClient = new Client(dbConfig);

      try {
        await pgClient.connect();
        const query = 'INSERT INTO inventory(inventory_id, inventory_details) VALUES($1, $2) RETURNING *';
        const values = [inventory_id, JSON.stringify(inventory_details)];
